...
**4. Weather / mood.** Nothing else settled by these designs except: the overcast grade itself lands in Slice 2; `NIGHT_WASH` (main.gd) is the only night tunable and `light_look.gd` is never edited for looks; the bright shipped constants that will clash with the muted grade — window glass `#7ec8e8` foremost — are **this slice's material**, named so nobody "fixes" them inside a sprite batch; property-lane gating (lane-5 style: saturation/value bounds with a proves-it-can-say-no old-value negative) is the established pattern to reuse.

...
Planning for 01–10 is locked. Execution order:
...
7. **Specified, not built:** v11 save round-trip (`godot:m2:save`) — see [Save/load + determinism for alpha systems](issues/10-save-load-determinism.md). Build with 5–6; the stamp is one bump.

Frontier is the open unblocked tickets under `issues/` (tuning harness). Remaining fog is in **Not yet specified**.

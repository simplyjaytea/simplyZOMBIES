...
func _run() -> void:
	...
	if ok:
		print("M2_SAVE_OK v20 ticket10 needs-era despawn-clean")
		quit(0)
	else:
...
func _version() -> bool:
	if int(SimSerialize.SAVE_VERSION) != 20:
		push_error("SAVE_VERSION %d want 20" % int(SimSerialize.SAVE_VERSION))
		return false

...
func _version() -> bool:
	if int(SimSerialize.SAVE_VERSION) != 30:
		push_error("SAVE_VERSION %d want 30 -- bump it here AND in check_m2_fortify.gd's _version(), which pins the same number" % int(SimSerialize.SAVE_VERSION))
		return false

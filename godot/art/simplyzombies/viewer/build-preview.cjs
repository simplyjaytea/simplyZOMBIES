...
const base = fs.existsSync(path.resolve(__dirname,'../manifest.json')) ? path.resolve(__dirname,'..') : path.resolve(__dirname, '../pack');
const manifestPath = path.resolve(process.argv[2] || path.join(base, 'manifest.json'));
const outputPath = path.resolve(process.argv[3] || path.join(base, 'simplyzombies-production-preview.html'));

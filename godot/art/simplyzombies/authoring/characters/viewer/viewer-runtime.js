(function () {
  ...
  const state = {zoom:2,paused:false,walkInPlace:false,autoTurn:false,turnStart:0,night:false,time:0,selected:M.assets[0]?.id,near:null,ready:false,lastStatus:'',pointer:null};
  ...
  const controls = {};
  ['loading','auto-turn','walk-cycle','pause','night','assets','asset-name','asset-meta','asset-count','scene-state','scale-note','motion-state','interact','interaction-hint','announcement'].forEach(id => controls[id] = document.getElementById(id));
  ...
  function framesFor(a,anim,dir) {
    ...
    result = result || f['idle_'+dir] || f.idle?.[dir] || f.idle || f.walk || [0];
  ...
  function frame(a,anim='idle',dir='e',time=state.time,fpsOverride) {
    ...
    let item = seq[Math.floor(time*fps)%seq.length] ?? 0;
  ...
  canvas.addEventListener('pointerdown',event=>{pointerAim(event);canvas.focus({preventScroll:true});});
  ...
  window.addEventListener('keydown',event=>{
    ...
    if(['w','a','s','d',' ','e'].includes(key)){event.preventDefault();if(key===' '&&!event.repeat)togglePause();else if(key==='e'&&!event.repeat)interact();else keys.add(key);}
  });
  ...
  Promise.all(Object.entries(B.images).map(([key,info])=>new Promise((resolve,reject)=>{
    ...
    window.ArtSample={manifest:M,state,player,npcs,frame,framesFor,byId,enabled,drawWorld,drawActor,drawAsset,renderInspect,update,viewport,selectAsset,interact,togglePause,setFacing,direction,actorDirection,blocked};

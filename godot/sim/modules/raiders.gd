...
# A band of bodies, one archetype draw and one `spawn` per position. This is the one place that
# rolls an archetype and spawns a raider for it -- `_emit_band` (a night's draw, positions off the
# district's edges) and the F8 dev menu (positions ahead of the player) are its only two callers,
# so a raider a tester spawns is a raider the director could have spawned, never a second recipe
# for the same body. Callers stamp their own `raidId`/`bandSize` afterwards with `stamp_band`;
# this only rolls and places.
#
# `positions` is a plain `Array` of `Vector2` rather than a typed one, so a caller building it out
# of a `Vector2i` pool (`_emit_band`'s tiles) or a `Vector2` line (the dev menu's spread) both pass
# without a cast. A position `spawn` refuses (an archetype nothing implements) is dropped rather
# than retried, exactly as `_emit_band` already dropped it -- a band short one body is the visible
# consequence, not a silently reshuffled draw.
static func spawn_band(world: Variant, positions: Array, rng: Variant) -> Array:
	var members: Array = []
	for p in positions:
		var pos: Vector2 = p
		var type_id: String = pick_type(world, rng)
		var ent: int = spawn(world, pos.x, pos.y, type_id)
		if ent >= 0:
			members.append(ent)
	return members

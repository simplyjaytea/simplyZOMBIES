...
const READ_KEYS: Array[String] = ["equipSlot", "melee", "ranged", "armor", "container", "food", "drink", "fuel", "light", "modification", "ammo", "filter", "buildMaterial", "warmth", "shedsRain", "lightFuel", "cooksInto", "boilsInto", "purifies", "noise", "bedQuality", "hygiene"]
...
func _the_catalogue_is_findable_and_read() -> bool:
	...
	for id in by_id.keys():
		...
		if String(e.get("class", "")) == "tool" and not (e.has("light") or e.has("modification") or e.has("noise") or e.has("bedQuality")):
			push_error("CATALOGUE: %s is a tool with no light, no modification, no noise and no bedQuality block -- a tool nothing reads" % String(id))
			return false
